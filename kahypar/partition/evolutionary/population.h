...
class Population {
 public:
  ...
  // TODO(robin): move to evo::mutate and rename /refactor
  ...
  // TODO(robin): rename individuum() and make const reference
  inline Individuum & getIndividuum(unsigned position);

  // TODO(robin): move to evo::select namespace and make it a free function
  inline std::size_t getRandomIndividuum();
  // TODO(robin): refactor /remove
  inline void insertIndividuumFromFile(std::string filename);
  // TODO(robin): move to evo::select::singleIndividualViaTournamet namespace and make it a free function
  inline Individuum getIndividuumTournament();
  // TODO(robin): move to evo::select::singleIndividualViaTournamet namespace and make it a free function
  inline unsigned getIndividuumTournamentPosition();
  // TODO(robin): move to evo::select:: and rename
 ...
 private:
  ...
  // TODO(robin): make const or remove/replace
...
inline std::vector<unsigned> Population::randomPositions(unsigned amount) {
  ...
  for (unsigned i = 0; i < _internalPopulation.size() && i < amount; ++i) {
    ret.push_back(i);
  }
  return ret;
}
